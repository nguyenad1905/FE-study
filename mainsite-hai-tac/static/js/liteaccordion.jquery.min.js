...
(function(b) {
    var a = function(g, l) {
        var i = { containerWidth: 630, containerHeight: 200, headerWidth: 65, activateOn: "click", firstSlide: 1, slideSpeed: 400, onTriggerSlide: function(m) {}, onSlideAnimComplete: function() {}, autoPlay: true, pauseOnHover: false, cycleSpeed: 1000, easing: "swing", theme: "basic", rounded: false, enumerateSlides: false, linkable: false },
            h = b.extend({}, i, l),
            ...
            e = {
                ...
                //     if (d.playing) { return }
                //     d.playing = setInterval(function() { j.eq(n()).trigger("click.liteAccordion") }, h.cycleSpeed)
                // },
                stop: function() {
                ...
                debug: function() { return { elem: g, defaults: i, settings: h, methods: e, core: d } }
            },
            d = {
                setStyles: function() {
                ...
                setSlidePositions: function() {
                    ...
                    if (!m.length) { j.eq(h.firstSlide - 1).addClass("selected") }
                    j.each(function(n) {
                        var q = b(this),
                            p = n * h.headerWidth,
                            o = j.first().next(),
                            r = parseInt(o.css("marginLeft"), 10) || parseInt(o.css("marginRight"), 10) || 0;
                        if (m.length) { if (n > j.index(m)) { p += f } } else { if (n >= h.firstSlide) { p += f } }
                ...
                bindEvents: function() { if (h.activateOn === "click") { j.on("click.liteAccordion", d.triggerSlide) } else { if (h.activateOn === "mouseover") { j.on("click.liteAccordion mouseover.liteAccordion", d.triggerSlide) } } if (h.linkable) { b(window).on("hashchange.liteAccordion", function(n) { var m = c.filter(function() { return b(this).attr("data-slide-name") === window.location.hash.split("#")[1] }); if (m.length) { d.triggerSlide.call(m.children("h4")[0], n) } }) } if (h.pauseOnHover && h.autoPlay) { g.on("mouseover.liteAccordion", function() { d.playing && e.stop() }).on("mouseout.liteAccordion", function() {!d.playing && e.play(d.currentSlide) }) } },
                currentSlide: h.firstSlide - 1,
                nextSlide: function(m) { var n = m + 1 || d.currentSlide + 1; return function() { return n++ % k } },
                playing: 0,
                slideAnimCompleteFlag: false,
                triggerSlide: function(o) {
                    var n = b(this),
                        m = { elem: n, index: j.index(n), next: n.next(), prev: n.parent().prev().children("h4"), parent: n.parent() };
                    if (h.linkable && m.parent.attr("data-slide-name")) { if (m.parent.attr("data-slide-name") !== window.location.hash.split("#")[1]) { return window.location.hash = "#" + m.parent.attr("data-slide-name") } }
                    d.currentSlide = m.index;
                    d.slideAnimCompleteFlag = false;
                    h.onTriggerSlide.call(m.next, n);
                    if (n.hasClass("selected") && n.position().left < f / 2) { d.animSlide.call(m) } else { d.animSlideGroup(m) }
                    if (h.autoPlay) {
                        e.stop();
                ...
                animSlide: function(m) {
                    var n = this;
                    if (typeof this.pos === "undefined") { this.pos = f }
                ...
                animSlideGroup: function(m) {
                    ...
                    b.each(n, function(o, p) {
                        ...
                        c.filter(r).children("h4").each(function() {
                            var t = b(this),
                                s = { elem: t, index: j.index(t), next: t.next(), prev: t.parent().prev().children("h4"), pos: q };
                ...
                ieClass: function(m) {
                    if (m < 7) { e.destroy() }
                    if (m >= 10) { return }
                    if (m === 7 || m === 8) { c.each(function(n) { b(this).addClass("slide-" + n) }) }
                    g.addClass("ie ie" + m)
                },
                init: function() {
                    ...
                    d.setStyles();
                    d.bindEvents();
                    if (h.cycleSpeed < h.slideSpeed) { h.cycleSpeed = h.slideSpeed }
                    h.autoPlay && e.play()
                }
            };
        d.init();
    ...
    b.fn.liteAccordion = function(e) {
        ...
        } else { if (typeof e === "string" && c[e]) { if (e === "debug") { return c[e].call(d) } else { c[e].call(d); return d } } }
    }
})(jQuery);